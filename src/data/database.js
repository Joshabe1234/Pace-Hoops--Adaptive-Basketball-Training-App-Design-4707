...
export const removePlayerFromTeam = (teamId, playerId) => {
  ...
  if (team) {
    ...
    if (!team.blockedPlayerIds) team.blockedPlayerIds = [];
    if (!team.blockedPlayerIds.includes(playerId)) {
      team.blockedPlayerIds.push(playerId);
    }
    database.teams[teamId] = team;

    // Delete their team chat messages
    Object.keys(database.messages).forEach(msgId => {
      const msg = database.messages[msgId];
      if (msg.teamId === teamId && msg.senderId === playerId) {
        delete database.messages[msgId];
      }
    });
...
export const getTeamMessages = (teamId, limit = 50) => {
  forceReload();
  const blockedIds = database.teams[teamId]?.blockedPlayerIds || [];
  return Object.values(database.messages)
    .filter(m => m.teamId === teamId && !blockedIds.includes(m.senderId))
