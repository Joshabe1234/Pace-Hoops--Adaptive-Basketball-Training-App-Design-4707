...
const CoachChat = ({ user, team }) => {
  ...
  const openDMChat = (player) => {
    ...
    setChatMode('dm-chat');
    markConversationRead(user.id, player.id);
    const dms = getDirectMessages(user.id, player.id);
    setDmMessages(dms);
  };
            ...
            return (
              ...
              >
                ...
                <div className={`max-w-[75%]`}>
                  {!isMe && (
                    <p className="text-xs text-slate-400 mb-1">{sender?.name || 'Unknown'}</p>
                  )}
                  <div
                    className={`p-3 rounded-2xl ${
                      isMe
