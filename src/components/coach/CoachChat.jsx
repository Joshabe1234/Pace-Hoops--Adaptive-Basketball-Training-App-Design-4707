...
const CoachChat = ({ user, team }) => {
  ...
  // Build a map of userId → user for rendering sender names
  ...
  const openDMChat = async (player) => {
    setDmRecipient(player);
    setChatMode('dm-chat');
    const dms = await getDirectMessages(user.id, player.id);
    setDmMessages(dms);
    await markConversationRead(user.id, player.id);
  };
        ...
        <div className="p-4 bg-slate-800 border-b border-slate-700 flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              {dmRecipient.position && `#${dmRecipient.jerseyNumber} • ${dmRecipient.position}`}
            </p>
          </div>
        </div>
              ...
              return (
                <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[75%] ${isMe ? 'order-2' : ''}`}>
                    <div className={`p-3 rounded-2xl ${isMe ? 'bg-orange-500 text-white rounded-br-md' : 'bg-slate-700 text-white rounded-bl-md'}`}>
        ...
        <div className="p-4 bg-slate-800 border-t border-slate-700">
          <div className="flex space-x-2">
            ...
            <button
              ...
              className="p-3 bg-orange-500 text-white rounded-xl hover:bg-orange-400 disabled:opacity-50"
            >
      ...
      <div className="p-4 md:p-6 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400">Private conversations with players</p>
          </div>
              ...
              {conversations.map((conv) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <p className="font-medium text-white truncate">{conv.otherUser?.name || 'Unknown'}</p>
        ...
        {messages.length === 0 ? (
          ...
          messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                {!isMe && (
                  <div className="w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center text-white text-sm font-semibold mr-2 flex-shrink-0">
                    {sender?.name?.charAt(0)?.toUpperCase() || '?'}
                  </div>
                )}
                <div className="max-w-[75%]">
                  {!isMe && <p className="text-xs text-slate-400 mb-1">{sender?.name || 'Unknown'}</p>}
                  <div className={`p-3 rounded-2xl ${isMe ? 'bg-orange-500 text-white rounded-br-md' : 'bg-slate-700 text-white rounded-bl-md'}`}>
                    <p className="text-sm">{msg.content}</p>
                  </div>
                  <p className={`text-xs text-slate-500 mt-1 ${isMe ? 'text-right' : ''}`}>{formatTime(msg.createdAt)}</p>
