...
const Navigation = ({ user, team, currentView, setCurrentView, onLogout, isCoach }) => {
  ...
  // For coaches (7 items), always keep chat visible so the unread badge shows on mobile.
  // Show: home, assignments, roster, chat, stats — health/schedule accessible via hamburger.
  const mobileNavItems = isCoach
    ? navItems.filter(item => ['home', 'assignments', 'roster', 'chat', 'stats'].includes(item.id))
    : navItems;
  ...
  return (
    <>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-slate-800 border-t border-slate-700 z-50 safe-area-bottom" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
      ...
      <header className="md:hidden fixed top-0 left-0 right-0 bg-slate-800 border-b border-slate-700 z-40 safe-area-top" style={{ paddingTop: 'env(safe-area-inset-top, 0px)' }}>
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center space-x-2 min-w-0 flex-1">
            ...
            <div className="min-w-0">
              <p className="font-bold text-white truncate leading-tight">Pace Hoops</p>
              <p className="text-xs text-slate-400 truncate">{isCoach ? 'Coach' : 'Player'}</p>
            </div>
          </div>
